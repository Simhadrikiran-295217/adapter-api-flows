...
The Party Lifecycle Management BIAN service, upon receiving the request, forwards it to the FinX Glue Adapter service to begin the Jumio verification workflow.

---

### 3. Step 1 — Jumio Account API (Integration to Jumio via Glue)

**API:** `POST https://account.emea-1.jumio.ai/api/v1/accounts`

The FinX Glue Adapter transforms and forwards the request to Jumio's Account Creation API with the following details:
- User consent
...
The Customer Channel uploads the front side of the identity document. The FinX Glue Adapter forwards this to Jumio as a multipart request using the upload URL returned in Step 1.
...
The back side of the identity document is uploaded similarly. The FinX Glue Adapter forwards it to Jumio as a multipart request.
...
Once both FRONT and BACK images are uploaded, the FinX Glue Adapter calls Jumio's Finalize Workflow API to signal that all credential parts are ready and verification can proceed.
...
The FinX Glue Adapter retrieves the full verification result from Jumio using the account ID and workflow execution ID.
...
**`200 OK`** — The FinX Glue Adapter returns the complete Initiate response (assessment result + final decision + document reference) through the Party Lifecycle Management SD back to the Customer Channel.
